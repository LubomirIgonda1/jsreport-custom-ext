...
module.exports = (reporter, definition) => {
  ...
  if (reporter.documentStore.model.entityTypes.TemplateType) {
    reporter.documentStore.model.entityTypes.TemplateType.pdfOperations2 = { type: 'Collection(jsreport.PdfOperationType)' }
  ...
  reporter.afterTemplatingEnginesExecutedListeners.add('pdf-utils2', (req, res) => {
  ...
  reporter.beforeRenderListeners.insert({ after: 'data' }, 'pdf-utils2', async (req, res) => {
    ...
    function pdfFormField (el) {
      ...
      return `<span class='jsreport-pdf-utils2-form-element jsreport-pdf-utils2-hidden-element' style='font-family: Helvetica;display: inline-block;vertical-align:middle;text-transform:none;font-size:1.1px;width: ${el.width}px; height: ${el.height}px'>form@@@${value}@@@</span>`
    }
    ...
    function pdfCreatePagesGroup (groupId) {
      ...
      const result = `<span class='jsreport-pdf-utils2-page-group jsreport-pdf-utils2-hidden-element' style='font-family: Helvetica;position:absolute;text-transform: none;opacity: 0.01;font-size:1.1px'>group@@@${value}@@@</span>`
    ...
    function pdfAddPageItem (item) {
      ...
      // we use position: absolute to make the element to not participate in flexbox layout
      // (making it not a flexbox child)
      const result = `<span class='jsreport-pdf-utils2-page-item jsreport-pdf-utils2-hidden-element' style='font-family: Helvetica;position:absolute;text-transform: none;opacity: 0.01;font-size:1.1px'>item@@@${value}@@@</span>`
  ...
  reporter.afterRenderListeners.insert(0, 'pdf-utils2', async (req, res) => {
    if (
      req.template.pdfPassword == null &&
      req.template.pdfMeta == null &&
      req.template.pdfSign == null &&
      (!req.template.pdfOperations2 || req.template.pdfOperations2.length === 0) &&
      !req.context.pdfUtilsOutlines &&
    ...
    if (isPreviewRequest && pdfPassword != null) {
      reporter.logger.debug('Skipping pdf-utils2 password addition, the feature is disabled during preview requests', req)
      pdfPassword = null
    }

    if (isPreviewRequest && pdfSign != null) {
      reporter.logger.debug('Skipping pdf-utils2 signature addition, the feature is disabled during preview requests', req)
      pdfSign = null
    }

    reporter.logger.info('pdf-utils2 is starting pdf processing', req)

    const result = await reporter.executeScript(
      {
        pdfContent: res.content.toString('base64'),
        operations: req.template.pdfOperations2 || [],
      ...
      },
      {
        ...
        timeout: -1,
        timeoutErrorMessage: 'Timeout during execution of pdf-utils2 operations',
    ...
    if (result.error) {
      ...
      throw reporter.createError('Error while executing pdf-utils2 operations', {
        original: error,
        weak: true
      })
    }
    ...
    reporter.logger.info('pdf-utils2 pdf processing was finished', req)
  })

  reporter.initializeListeners.add('pdf-utils2', async (req, res) => {
